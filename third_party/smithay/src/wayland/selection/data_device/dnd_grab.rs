                ...
                if let Some(ref source) = self.data_source {
                        ...
                        .filter(|d| d.id().same_client_as(&surface.id()))
                    {
                        ...
                        with_source_metadata(source, |meta| {
                            ...
                            // `source_actions` does not exist before version 3, and the
                            // offer was created at the data device's version just above.
                            // Sending it to a version 1 or 2 client is a protocol
                            // violation that libwayland answers by aborting the client:
